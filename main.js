...
function handleClick(e) {
    ...
    if (element.id === "remove") {
        ...
        // parentElement: tıklanılan elamanın kapsayıcısına ulaşma (kapsayıcısının kapsayıcısı)
        const wrapper = element.parentElement.parentElement
        // console.log(wrapper)

        // silinen elamanın fiyatını alma
        const deletePrice = wrapper.querySelector("#value").innerText
        Number(deletePrice.innerText);

        // silinenin fiyatını toplamdan çıkarma updateTotal'i güncelleme
...
function handleFilter(e) {
    ...
    // ! childNodes :
    // childNodes : parentElement elementin tersine kapsayıcıya 
